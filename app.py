...
# # Setup memory (you can keep this outside the route or per user session)
# memory = ConversationBufferMemory(memory_key="chat_history", return_messages=True, output_key='result')

# # Modify your prompt template to include chat history
# prompt_template = """You are an agriculture expert bot. Be concise and simple.
# {chat_history}
# CONTEXT: {context}
# QUESTION: {question}"""
...
# def setup_retrieval_qa_with_memory(db):
#     retriever = db.as_retriever(similarity_score_threshold=0.6)
#     chain = RetrievalQA.from_chain_type(
#         llm=llm,
#         chain_type='stuff',
...
#     web_chunks = list(chain.from_iterable(split_text(content) for content in contents))
#     db = FAISS.from_texts(web_chunks, embedding_function)
#     return db
...
# Modify your prompt template to include chat history
prompt_template = """You are an agriculture expert bot.  
Answer clearly and structure responses with bullet points or numbered lists when appropriate.  
Keep explanations concise (under 100 words) and simple.  
If unknown, reply with 'Don't know.'  
{chat_history}
CONTEXT: {context}
QUESTION: {question}"""
...
QUESTION: {question}"""
    ...
    PROMPT = PromptTemplate(template=f"{prompt_template}", input_variables=["question"])
...
async def translate_from_english(result, target_lang):
    try:
        if target_lang != "en":
            translator = Translator()
            translation = await translator.translate(result, src='en', dest=target_lang)
            return translation.text
        return result
    except Exception as e:
        return str(e)
...
def index():
    if request.method == 'POST':
        try:
            query = request.form['query']
            translated_query, detected_lang = asyncio.run(translate_text(query))
    ...
    "query": translated_query      # if no extra context, leave empty
})
                ...
                translated_response = asyncio.run(translate_from_english(result, detected_lang))
                return jsonify({'response': translated_response, 'status': 'success'})
            else:
                return jsonify({'response': 'Error: chain is not callable', 'status': 'error'})
        except Exception as e:
